...
static void set_segment(PlayClockDisplay *display, uint8_t digit,
                        segment_t segment, bool on);
static void set_segment_color(PlayClockDisplay *display, uint8_t digit,
                              segment_t segment, uint8_t r, uint8_t g,
                              uint8_t b);
static void clear_display(PlayClockDisplay *display);
static void update_led_strip(PlayClockDisplay *display);
static void display_digit(PlayClockDisplay *display, uint8_t digit,
                          uint8_t value);
...
static void init_segment_mapping(PlayClockDisplay *display) {
  ...
  for (int digit = 0; digit < PLAY_CLOCK_DIGITS; digit++) {
    for (int segment = 0; segment < SEGMENTS_PER_DIGIT; segment++) {
      display->segments[digit][segment].start =
          digit * 450 + segment * 60; // Mock positions
      display->segments[digit][segment].count =
          50; // Mock LED count per segment
    }
  }
}
...
static void set_segment_color(PlayClockDisplay *display, uint8_t digit,
                              ...
                              uint8_t b) {
  // Mock implementation - just log
  ESP_LOGD(TAG, "Set segment color: digit=%d, segment=%d, RGB=(%d,%d,%d)",
           digit, segment, r, g, b);
}
